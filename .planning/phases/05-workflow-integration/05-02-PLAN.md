...
**3. Drive file picker -- handler function:**
For each expected file slot, add a "Pick from Drive" button alongside the existing "Upload" button. Use `useDriveFilePicker` hook to get `openPicker`.

Create a handler function that wires the picker result into component state:

```typescript
const handleDriveFileSelect = async (expectedFile: FileDefinition, pickedFile: DrivePickerFile) => {
  // 1. Set loading state
  setDriveFiles(prev => ({
    ...prev,
    [expectedFile.id]: { driveFileId: pickedFile.id, driveMimeType: pickedFile.mimeType, name: pickedFile.name, validated: false, isLoading: true }
  }))

  // 2. Clear any existing local upload for this slot
  setUploadedFiles(prev => { const next = { ...prev }; delete next[expectedFile.id]; return next })
  ...
  // 5. Validate columns against expected columns (reuse validateFileColumns)
  const validated = validateFileColumns(downloadResult.columns, expectedFile.expectedColumns)
  ...
  setDriveFiles(prev => ({
    ...
    [expectedFile.id]: {
      ...
      driveModifiedTime: pickedFile.modifiedTime,
      name: pickedFile.name,
      validated,
      columns: downloadResult.columns,
...
The "Pick from Drive" button calls `openPicker` and passes `handleDriveFileSelect` as the onSelect callback, binding the current `expectedFile`. Wrap the entire body in try/catch -- on error, set `error` field in `DriveRunFileState` and `isLoading: false`.

Similarly, when user uploads a local file for a slot, clear `driveFiles[slotId]`:
```typescript
// Inside existing local file upload handler, add:
setDriveFiles(prev => { const next = { ...prev }; delete next[expectedFile.id]; return next })
```
