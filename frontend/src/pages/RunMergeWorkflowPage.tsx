...
import { mergeWorkflowApi, fileApi, type MergeRunResult } from '../lib/api'
...
export function RunMergeWorkflowPage() {
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      // Call the merge API
      const mergeResult = await mergeWorkflowApi.run(id, filesToSend, fileConfigs)
      
      setResult({
        success: true,
        message: `Merge completed successfully! ${mergeResult.rowCount} rows generated.`,
        runId: mergeResult.runId,
        rowCount: mergeResult.rowCount,
        columns: mergeResult.columns,
        previewData: mergeResult.previewData,
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        ...
        <p className="text-sm text-slate-500 mb-4">
          Upload the files that match the expected structure. The workflow will merge them based on your configuration.
        </p>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {result.success && result.runId && id && (
                  <a
                    href={mergeWorkflowApi.downloadUrl(id, result.runId)}
                    download
